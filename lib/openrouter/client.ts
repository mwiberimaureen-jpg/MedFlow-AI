...
/**
 * Strip markdown fences and any prose the model wrapped around a JSON object
 * (e.g. "Based on the conditions provided, here is..." before the `{`).
 */
...

{
  ...
  "summary": "TRANSCRIPT of the history — a complete, faithful report of everything clinically documented. Never interpret, never add, never omit. REQUIRED ORDER: 1) '[Name], [age]-year-old [sex][, parity if OB/GYN], Day [N] of admission.' 2) Chief complaint — include EVERY detail that characterises it: total duration of illness, number of episodes, duration of EACH episode, estimated volume or severity (e.g. soaking pads, number of clots, amount of blood), triggers, relieving and aggravating factors, and ALL associated features explicitly mentioned (e.g. tracheal deviation, photophobia, neck stiffness, fecal or urinary incontinence during episodes, post-ictal state, loss of consciousness, cyanosis, tongue biting, eye deviation — include ONLY what is documented). 3) Full HPI narrative — all documented symptoms, their time course, and progression. 4) Relevant PMH and PSH — documented chronic conditions, current long-term medications with dose/route/frequency, relevant surgical history. 5) Examination findings exactly as documented. 6) Investigations SENT — list every test sent; do NOT include result values or interpret them (results belong in test_interpretation). 7) ALL procedures and treatments given — every drug with dose/route/frequency and duration, every procedure performed (e.g. nasal packing, IV access, Foley catheter, NG tube, airway manoeuvres). 8) Clinician's stated plan. ABSOLUTE RULES: If it is in the history, it MUST be in the summary — especially episode counts, durations, and associated clinical features. Never compress to the point of dropping documented detail. Never add anything not in the history. No impressions, diagnoses, differentials, or result values. Use 1-3 concise paragraphs.",
  "todo_items": [
    {
...
export async function analyzeDailyProgress(
  ...
  const userMessage =
    ...
    `ABSOLUTE RULES: If it is in the progress notes, it MUST appear in the summary — especially episode counts, durations, and associated clinical features. Do NOT add impressions, diagnoses, or interpretations. Do NOT add anything not in the notes. Do NOT compress to the point of losing documented clinical detail.\n` +
    `LENGTH: 1-2 concise paragraphs. NEVER add impressions or management plans.\n\n` +
...
export async function generateLearningSpark(
  ...
  // with prose (e.g. "Based on the conditions provided..."), which used to
  // surface a raw JSON.parse SyntaxError to the user. Extract the {...}
  // object out of any surrounding prose, and retry once on a fully malformed
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try {
      return JSON.parse(extractJsonObject(rawContent)) as SparkContent
    } catch {
      console.warn(
        `generateLearningSpark: non-JSON response on attempt ${attempt}${attempt < 2 ? ', retrying' : ', giving up'}:`,
        rawContent.slice(0, 300)
      )
    }
  }

  throw new Error('The teaching model returned an unexpected response. Please try refreshing the spark.')
}
